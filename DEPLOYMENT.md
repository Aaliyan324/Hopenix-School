...
To remain strictly within Vercel's **Hobby Plan limit** (maximum 12 Serverless Functions per deployment), all API endpoints are consolidated into a **single Vercel Serverless Function entrypoint** (`/api/index.js`). Individual route handlers are organized in `/api/_lib/handlers/` and routed internally.
